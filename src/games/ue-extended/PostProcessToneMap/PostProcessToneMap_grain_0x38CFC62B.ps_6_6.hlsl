...
OutputSignature main(
  ...
  float _61 = float((int)(((int)(uint)((bool)(_49 > 0.0f))) - ((int)(uint)((bool)(_49 < 0.0f)))));
  float _62 = float((int)(((int)(uint)((bool)(_50 > 0.0f))) - ((int)(uint)((bool)(_50 < 0.0f)))));
  ...
  float film_grain_intensity_midtones = FilmGrainIntensityMidtones, film_grain_intensity_shadows = FilmGrainIntensityShadows, film_grain_intensity_highlights = FilmGrainIntensityHighlights;
#if 1
  ...
  float _469 = (((_447 - _459) * film_grain_intensity_midtones) + (film_grain_intensity_shadows * (1.0f - _447))) + (film_grain_intensity_highlights * _459);
  ...
  float _509 = exp2(log2((_431 * 0.009999999776482582f) * ((((_481.x * _436) + -1.0f) * _469) + 1.0f)) * 0.1593017578125f);
  float _510 = exp2(log2((_432 * 0.009999999776482582f) * ((((_481.y * _437) + -1.0f) * _469) + 1.0f)) * 0.1593017578125f);
  float _511 = exp2(log2((_433 * 0.009999999776482582f) * ((((_481.z * _438) + -1.0f) * _469) + 1.0f)) * 0.1593017578125f);

  float4 _550 = ColorGradingLUT.Sample(ColorGradingLUTSampler, float3(((LUTScale * exp2(log2((1.0f / ((_509 * 18.6875f) + 1.0f)) * ((_509 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset), ((LUTScale * exp2(log2((1.0f / ((_510 * 18.6875f) + 1.0f)) * ((_510 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset), ((LUTScale * exp2(log2((1.0f / ((_511 * 18.6875f) + 1.0f)) * ((_511 * 18.8515625f) + 0.8359375f)) * 78.84375f)) + LUTOffset)));
  float _554 = _550.x * 1.0499999523162842f;
  float _555 = _550.y * 1.0499999523162842f;
  float _556 = _550.z * 1.0499999523162842f;
  [branch]
  if (!(bOutputInHDR == 0)) {
    float _568 = (pow(_554, 0.012683313339948654f));
    float _569 = (pow(_555, 0.012683313339948654f));
    float _570 = (pow(_556, 0.012683313339948654f));
    float _603 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_568 + -0.8359375f)) / (18.8515625f - (_568 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _604 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_569 + -0.8359375f)) / (18.8515625f - (_569 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    float _605 = max(6.103519990574569e-05f, ((exp2(log2(max(0.0f, (_570 + -0.8359375f)) / (18.8515625f - (_570 * 18.6875f))) * 6.277394771575928f) * 10000.0f) / EditorNITLevel));
    _631 = min((_603 * 12.920000076293945f), ((exp2(log2(max(_603, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _632 = min((_604 * 12.920000076293945f), ((exp2(log2(max(_604, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
    _633 = min((_605 * 12.920000076293945f), ((exp2(log2(max(_605, 0.0031306699384003878f)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f));
  } else {
    _631 = _554;
    _632 = _555;
    _633 = _556;
  }
  float _640 = (frac(sin((TEXCOORD_2.w * 543.3099975585938f) + TEXCOORD_2.z) * 493013.0f) * 2.0f) + -1.0f;
  // clang-format off
  float _643 = min(max((_640 * +1.#INF), -1.0f), 1.0f);
  // clang-format on
  float _652 = (_643 - (sqrt(saturate(1.0f - abs(_640))) * _643)) * BackbufferQuantizationDithering;
  SV_Target.x = (_652 + _631);
  SV_Target.y = (_652 + _632);
  SV_Target.z = (_652 + _633);
  SV_Target.w = 0.0f;
  SV_Target_1 = dot(float3(_554, _555, _556), float3(0.29899999499320984f, 0.5870000123977661f, 0.11400000005960464f));
